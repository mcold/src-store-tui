...
func (pagePro *pageProType) build() {
	...
	pagePro.saveArea = tview.NewTextArea()
	pagePro.saveArea.SetBorderColor(tcell.ColorBlue)
	pagePro.saveArea.SetBorderPadding(1, 1, 1, 1)
	pagePro.saveArea.SetDisabled(true)

	pagePro.saveArea.SetBorder(true).
		SetBorderPadding(1, 1, 1, 1).
		SetBorderColor(tcell.ColorBlue).
		SetTitle("name").
		SetTitleAlign(tview.AlignLeft)

	pagePro.saveArea.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		if event.Key() == tcell.KeyEsc {
			hideProSave()
			return nil
		}
		if event.Rune() == 'v' && event.Modifiers() == tcell.ModAlt {
			...
			pagePro.saveArea.SetText(pagePro.saveArea.GetText()+clipBoardContent, true)
		}
		return event
	})
	...
	pagePro.flListPro.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		...
		if event.Key() == tcell.KeyCtrlQ {
			if pagePro.descArea.GetDisabled() == true {
				...
				if pagePro.saveArea.GetDisabled() == false {
					hideProSave()
				}
		...
		if event.Key() == tcell.KeyCtrlS {
			if pagePro.saveArea.GetDisabled() == true {
				if pagePro.descArea.GetDisabled() == false {
					hideProDesc()
				}
				if pagePro.nameArea.GetDisabled() == false {
				...
				pagePro.saveArea.SetTitle("save")
				pagePro.flListPro.AddItem(pagePro.saveArea, 0, 1, false)
				app.SetFocus(pagePro.saveArea)
				pagePro.saveArea.SetDisabled(false)
			} else {
				hideProSave()
			}
...
func hideProSave() {
	...
	pagePro.flListPro.RemoveItem(pagePro.saveArea)
	pagePro.lPro.Clear()
	setListPro()
	pagePro.lPro.SetCurrentItem(curPos)
	app.SetFocus(pagePro.lPro)
